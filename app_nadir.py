...
def compute_nadir_mission(aoi_payload, calc, flight_azimuth_deg=0.0,
                          lead_in_out_m=200.0, turn_time_min=3.5,
    ...
    lead_c     = lead_in_out_m * merc_scale

    # Rotate polygon to flight-line frame
    ...
    # Continue-over-gap threshold (same logic as oblique planner)
    gap_threshold_m = speed_ms * turn_time_min * 60.0
    gap_threshold_c = gap_threshold_m * merc_scale
    ...
    for x in offsets:
        ...
        # Merge or split segments based on gap threshold
        merged = [segs[0]]
        for seg in segs[1:]:
    ...
    # Rotate lines back to geographic frame
    def _rotate_pt(px, py, cx, cy, angle_deg):
    ...
    geo_lines = []
    for x0, y0, x1, y1 in mission_lines:
        gx0, gy0 = _rotate_pt(x0, y0, cx, cy, flight_azimuth_deg)
        gx1, gy1 = _rotate_pt(x1, y1, cx, cy, flight_azimuth_deg)
        geo_lines.append(((gx0, gy0), (gx1, gy1)))

    geo_transit = []
    for x0, y0, x1, y1 in transit_lines:
        gx0, gy0 = _rotate_pt(x0, y0, cx, cy, flight_azimuth_deg)
        gx1, gy1 = _rotate_pt(x1, y1, cx, cy, flight_azimuth_deg)
        geo_transit.append(((gx0, gy0), (gx1, gy1)))

    # Statistics
    line_count = len(geo_lines)
    line_lengths = [math.hypot(b[0]-a[0], b[1]-a[1]) for a, b in geo_lines]
    total_length_c = sum(line_lengths)
    total_length_m = total_length_c / merc_scale
    avg_length_m   = (total_length_m / line_count) if line_count > 0 else 0.0

    triggers_per_line = [max(1, int(math.floor(ll / trig_sp_c))) for ll in line_lengths]
    total_triggers = sum(triggers_per_line)

    # Storage
    cam = NADIR_CAMERAS[calc["camera_name"]]
    storage_per_image_mb = cam["storage_mb"].get(storage_profile, list(cam["storage_mb"].values())[0])
    total_storage_mb = total_triggers * storage_per_image_mb

    # Time
    airborne_s   = total_length_m / speed_ms
    turn_s_total = (line_count - 1) * turn_time_min * 60.0
    ...
    swath_area_m2 = line_count * (line_sp_m) * avg_length_m
    coverage_pct  = min(100.0, 100.0 * swath_area_m2 / area_m2) if area_m2 > 0 else 0.0
...
def optimise_heading(aoi_payload, calc, lead_in_out_m=200.0, turn_time_min=3.5,
                     step_deg=1):
    best_heading, best_lines, best_dist = 0.0, None, float("inf")
    for hdg in range(0, 180, step_deg):
        ...
        if result is None:
            continue
        dist = result["total_line_length_m"]
        if dist < best_dist:
            best_dist = dist
            best_heading = float(hdg)
            best_lines = result
    return best_heading, best_lines
...
def draw_mission_map(mission_outputs, calc, dist_unit="m"):
    ...
    if aoi_poly is not None:
        def _draw_poly(p):
            if getattr(p, "geom_type", "") == "Polygon":
                xs, ys = p.exterior.xy
                ax.fill(xs, ys, alpha=0.08, color="#4488ff")
                ax.plot(xs, ys, color="#4488ff", lw=1.5)
                for interior in p.interiors:
                    ix, iy = zip(*interior.coords)
                    ax.fill(ix, iy, alpha=0.25, color="#0f2235")
    ...
    for i, (a, b) in enumerate(geo_lines):
        ...
        ax.plot([a[0], b[0]], [a[1], b[1]], color=col, lw=0.8, alpha=0.85)

    # Transit-over-gap lines
    for a, b in transit:
        ax.plot([a[0], b[0]], [a[1], b[1]], color="#ff9944", lw=1.2,
                ls="--", alpha=0.7)

    # Footprint diagram for one sample line
    if geo_lines:
        mid_idx = len(geo_lines) // 2
        a, b = geo_lines[mid_idx]
        line_vec = (b[0]-a[0], b[1]-a[1])
        ln = math.hypot(*line_vec) or 1.0
        ux, uy = line_vec[0]/ln, line_vec[1]/ln        # along-track unit
        px_u, py_u = -uy, ux                            # across-track unit

        ms = mission_outputs.get("merc_scale", 1.0)
        hw = calc["fp_across_m"] * ms / 2.0
        lsp = calc["line_sp_m"] * ms
        tsp = calc["trig_sp_m"] * ms

        # Show 3 sample footprints
        for t_frac in (0.25, 0.50, 0.75):
            tx = a[0] + line_vec[0] * t_frac
            ty = a[1] + line_vec[1] * t_frac
            fl = calc["fp_along_m"] * ms / 2.0
            fp = [
                (tx + px_u*hw + ux*fl, ty + py_u*hw + uy*fl),
                (tx - px_u*hw + ux*fl, ty - py_u*hw + uy*fl),
                (tx - px_u*hw - ux*fl, ty - py_u*hw - uy*fl),
                (tx + px_u*hw - ux*fl, ty + py_u*hw - uy*fl),
            ]
            patch = mpatches.Polygon(fp, closed=True,
                                     facecolor="#3fb95018", edgecolor="#3fb950",
                                     lw=0.7, zorder=3)
            ax.add_patch(patch)

    scale_m = mission_outputs.get("merc_scale", 1.0)
    ...
    xt = ax.get_xticks()
    ax.set_xticklabels([f"{m_to_unit(t/scale_m, dist_unit):.0f}" for t in xt], color="#8b949e")
    yt = ax.get_yticks()
    ax.set_yticklabels([f"{m_to_unit(t/scale_m, dist_unit):.0f}" for t in yt], color="#8b949e")
    ...
    ax.set_title(
        f"Nadir flight plan — {lc} lines | {hdg:.0f}° | {dist/1000:.1f} km total",
...
def make_kml_export(mission_outputs, calc):
    ...
    def _wgs84(lx, ly):
        ...
        return lon, lat
    def _cs_line(a, b):
        lo0, la0 = _wgs84(a[0], a[1])
        lo1, la1 = _wgs84(b[0], b[1])
        return f"{lo0:.8f},{la0:.8f},0 {lo1:.8f},{la1:.8f},0"
    def _cs_ring(ring):
        return " ".join(f"{_wgs84(p[0], p[1])[0]:.8f},{_wgs84(p[0], p[1])[1]:.8f},0"
                        for p in ring)
    ...
    tsp_c       = tsp * merc_scale

    out = ['''<?xml version="1.0" encoding="UTF-8"?>
<kml xmlns="http://www.opengis.net/kml/2.2">
<Document>''']
    out.append(f'''  <name>{name} — Nadir Flight Plan</name>
  <description>Camera: {calc["camera_name"]} | FL: {calc["focal_mm"]} mm | GSD: {gsd_cm:.1f} cm | AGL: {agl_m:.0f} m | Azimuth: {hdg:.1f}° | Lines: {lc} | Line spacing: {lsp:.0f} m | Trigger spacing: {tsp:.0f} m | Total: {dist_km:.1f} km | Flying time: {fly_hr:.2f} hr</description>''')
  ...
  <Style id="transit"><LineStyle><color>ffff6644</color><width>2</width></LineStyle></Style>
  <Style id="aoi"><LineStyle><color>ff4488ff</color><width>3</width></LineStyle><PolyStyle><color>204488ff</color></PolyStyle></Style>
  <Style id="trig"><IconStyle><color>ff00ffff</color><scale>0.4</scale><Icon><href>http://maps.google.com/mapfiles/kml/shapes/placemark_circle.png</href></Icon></IconStyle><LabelStyle><scale>0</scale></LabelStyle></Style>''')
    ...
    for i, (a, b) in enumerate(geo_lines):
        ...
        lo0, la0 = _wgs84(a[0], a[1])
        lo1, la1 = _wgs84(b[0], b[1])
        out.append(f'''    <Placemark><name>Line {i+1}</name><styleUrl>#{sid}</styleUrl>
      <LineString><tessellate>1</tessellate><coordinates>{lo0:.8f},{la0:.8f},0 {lo1:.8f},{la1:.8f},0</coordinates></LineString></Placemark>''')
    out.append('  </Folder>')

    # 1b. Transit gaps
    if transit:
        out.append('''  <Folder>\n    <name>1b. Transit (cameras off)</name>\n    <visibility>1</visibility>''')
        for a, b in transit:
            lo0, la0 = _wgs84(a[0], a[1])
            lo1, la1 = _wgs84(b[0], b[1])
    ...
    for a, b in geo_lines:
        line_vec = (b[0]-a[0], b[1]-a[1])
        ln = math.hypot(*line_vec)
        if ln < 1:
            continue
        ux, uy = line_vec[0]/ln, line_vec[1]/ln
        n_trigs = max(1, int(math.floor(ln / tsp_c)))
        for ti in range(n_trigs):
            t = (ti + 0.5) / n_trigs
            tx, ty = a[0] + line_vec[0]*t, a[1] + line_vec[1]*t
            lo, la = _wgs84(tx, ty)
            out.append(f'    <Placemark><styleUrl>#trig</styleUrl><Point><coordinates>{lo:.8f},{la:.8f},0</coordinates></Point></Placemark>')
    out.append('  </Folder>')

    # 3. AOI boundary
    if aoi_poly is not None:
        out.append('''  <Folder>\n    <name>3. AOI Boundary</name>\n    <visibility>1</visibility>''')
        def _add_poly_pm(p):
            if getattr(p, "geom_type", "") == "Polygon":
                ext_cs = _cs_ring(list(p.exterior.coords))
                out.append(f'''    <Placemark><styleUrl>#aoi</styleUrl>
      <Polygon><tessellate>1</tessellate>
        <outerBoundaryIs><LinearRing><coordinates>{ext_cs}</coordinates></LinearRing></outerBoundaryIs>''')
                for interior in p.interiors:
                    inner_cs = _cs_ring(list(interior.coords))
                    out.append(f'        <innerBoundaryIs><LinearRing><coordinates>{inner_cs}</coordinates></LinearRing></innerBoundaryIs>')
                out.append('      </Polygon></Placemark>')
            elif hasattr(p, "geoms"):
                for sub in p.geoms:
                    _add_poly_pm(sub)
        _add_poly_pm(aoi_poly)
        out.append('  </Folder>')
...
def draw_footprint_diagram(calc, dist_unit="m", n_strips=3, n_triggers=4):
    ...
    # Dimension arrows
    half = n_strips//2 * ls + fp_w/2 + ls * 0.1
    ax.annotate("", xy=(ls, -fp_h * (n_triggers//2 + 0.7)),
                xytext=(0.0, -fp_h * (n_triggers//2 + 0.7)),
                arrowprops=dict(arrowstyle="<->", color="#d29922", lw=1.2))
    ax.text(ls/2, -fp_h * (n_triggers//2 + 0.9),
            f"Line sp: {m_to_unit(ls, dist_unit):.0f} {dist_unit}\n"
            f"Sidelap: {calc['side_pct']}%",
            color="#d29922", ha="center", va="top", fontsize=8)

    ax.annotate("", xy=(fp_w * (n_strips//2 + 0.8), ts),
                xytext=(fp_w * (n_strips//2 + 0.8), 0.0),
                arrowprops=dict(arrowstyle="<->", color="#58a6ff", lw=1.2))
    ax.text(fp_w * (n_strips//2 + 1.1), ts/2,
            f"Trig sp: {m_to_unit(ts, dist_unit):.0f} {dist_unit}\n"
            f"Fwdlap: {calc['fwd_pct']}%",
            color="#58a6ff", ha="left", va="center", fontsize=8)

    pad = max(fp_w, fp_h) * 0.5
    ax.set_xlim(-(n_strips//2)*ls - fp_w - pad, (n_strips//2)*ls + fp_w + pad*3)
    ax.set_ylim(-(n_triggers//2)*ts - fp_h - pad*2, (n_triggers//2)*ts + fp_h + pad)
...
if not SHAPELY_AVAILABLE:
    ...
    with opt_col:
        ...
        lead_in_m    = st.number_input("Lead-in/out per line (m)", value=st.session_state.nadir_lead_in_m,
                                       min_value=0.0, max_value=2000.0, step=50.0, key="lead_m")
    ...
    with hdg_col1:
        use_optimise = st.checkbox("Auto-optimise heading (minimum flight distance)", value=False, key="opt_hdg")
    with hdg_col2:
        if not use_optimise:
    ...
    if mission is not None:
        ...
        with m4:
            ...
            st.metric("Coverage est.",   f"{mission['coverage_pct']:.0f}%")
        with m5:
        ...
        map_fig = draw_mission_map(mission, calc, dist_unit=dist_unit)
        if map_fig:
            st.pyplot(map_fig)
